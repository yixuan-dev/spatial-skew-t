...
\author{}
\date{July 2026 --- interim report v4 (settings 1--12, the
threshold-vs-symmetry disambiguation, the $K \ge 30$ extension, and the
non-$t$ extension 13--21; settings 16 and 20 at $n = 30$)}
...
New in v4: the nine non-$t$ extension settings (13--21; Gaussian-GP errors,
so the skew-$t$ error law is misspecified) are fitted and included in the
score tables --- settings 16 and 20 at $n = 30$ on the full $K$ grid, the
other seven as $n = 3$ pilots; their systematic analysis is deferred.
\end{abstract}

\section{Design}

\paragraph{Data-generating process.}
Settings 1--12 share the skew-$t$-1 core on $n_s=144$ sites drawn uniformly on
$[0,10]^2$ (settings 13--21 replace it with a Gaussian process; see below):
\begin{equation}
...
any pooled-$\beta$ mean basis). Settings 13--21 form a \emph{non-$t$ extension}:
they drop the skew-$t$ generator entirely in favour of a nonlinear mean surface
(sd 11) with Gaussian-GP errors ($\sigma_g = 4$, same $C_{\mathrm{exp}}$), so
that the mean channel is tested without the crutch of a correctly specified
error law at large $K$. All nine are fitted and scored and appear in the score
tables --- settings 16 and 20 at $n = 30$ on the full $K$ grid, the other seven
as $n = 3$ pilots ($\Kmrts \in \{0, 30, 40, 50\}$);
Section~\ref{sec:spotlight} introduces four of them in detail, and the
systematic analysis is deferred to the next revision.

\begin{table}[ht]
\centering\small
\caption{The DGP settings and the number of simulation replicates
$n$ (independent datasets) fitted per setting. Settings 1--12 share the
skew-$t$ core; settings 13--21 are the non-$t$ extension (Gaussian-GP errors,
$\sigma_g = 4$; nonlinear mean surface orthogonalised against $[1,s_1,s_2]$
and standardised to $\mathrm{sd}=11$). Routes: \emph{mean} = added to
...
\midrule
\multicolumn{5}{l}{\emph{Non-$t$ extension (Gaussian-GP errors; skew-$t$ generator dropped)}$^{b}$} \\
13 & franke         & mean, Franke (multiscale)                 & mean & 3 \\
14 & ridge\_curved  & mean, curved Gaussian ridge               & mean & 3 \\
15 & annulus        & mean, Ricker ring ($r_0{=}3$, $w{=}1.5$)   & mean & 3 \\
16 & sigmoid\_front & mean, sigmoid front ($\tanh$)             & mean & 30 \\
17 & gp\_mean       & mean, GP draw (per dataset, fixed in $t$)  & mean & 3 \\
18 & franke\_sas    & mean $=$ setting 13; sinh-arcsinh error   & mean & 3 \\
19 & poly2          & mean, bowl $+$ saddle (2nd-order poly)     & mean & 3 \\
20 & transform\_mix & mean, log $+$ exp-decay $+$ ratio terms    & mean & 30 \\
21 & nonsmooth      & mean, hinge $+\,|\cdot|$ ($C^0$ creases)   & mean & 3 \\
\bottomrule
\multicolumn{5}{p{0.92\textwidth}}{\footnotesize $^{a}$ setting 2 was fitted at $K=0$ only (no MRTS sweep); it is excluded from the score tables. $^{b}$ settings 13--21: settings 16 and 20 at $n=30$ on the full $K$ grid, the other seven as $n=3$ pilots on $\Kmrts \in \{0,30,40,50\}$.}
\end{tabular}
\end{table}

\begin{table}[ht]
\centering\footnotesize
\caption{Anatomy of the DGPs: marginal law of $Y_t(\bs)$, spatial
...
($\sigma_{\mathrm{add}} = 3$); $f_1, f_2$ are the two cosine bumps. For the
non-$t$ block (13--21) the error is $\sigma_g\,\varepsilon$ with
$\varepsilon \sim N(0, C_{\mathrm{exp}})$, $\sigma_g = 4$ (so the marginal is
Gaussian, sd 4), and every mean surface is orthogonalised against
$[1,s_1,s_2]$ and rescaled to $\mathrm{sd} = 11$.}
\label{tab:anatomy}
...
\midrule
\multicolumn{5}{l}{\emph{Non-$t$ extension: Gaussian-GP errors, $\sigma_g = 4$, mean surface sd 11}} \\
13 & franke         & Gaussian & $C_{\mathrm{exp}}$, stationary & $10\,+$ Franke's function (2 large $+$ 1 medium bump, 1 fine dip) \hfill (11.0) \\
14 & ridge\_curved  & Gaussian & $C_{\mathrm{exp}}$, stationary & $10\,+$ curved Gaussian ridge $\exp[-(s_2' - 0.5 - 0.25\sin 2\pi s_1')^2 / 2\cdot0.12^2]$ \hfill (11.0) \\
15 & annulus        & Gaussian & $C_{\mathrm{exp}}$, stationary & $10\,+$ Ricker ring $\psi((r-3)/1.5)$, $r = \lVert\bs - (5,5)\rVert$ \hfill (11.0) \\
16 & sigmoid\_front & Gaussian & $C_{\mathrm{exp}}$, stationary & $10\,+ \tanh[(\lVert\bs - (2,2)\rVert - 4.5)/1.0]$ sigmoid front \hfill (11.0) \\
17 & gp\_mean       & Gaussian & $C_{\mathrm{exp}}$, stationary & $10\,+$ GP draw (Mat\'ern $\nu{=}1.5$, range 2), redrawn per dataset, fixed in $t$ \hfill (11.0) \\
18 & franke\_sas    & sinh-arcsinh ($\epsilon{=}0.8$, $\delta{=}1$), right-skewed & $C_{\mathrm{exp}}$, stationary & $10\,+$ Franke (identical to setting 13) \hfill (11.0) \\
19 & poly2          & Gaussian & $C_{\mathrm{exp}}$, stationary & $10\,+$ 2nd-order polynomial (bowl $+$ saddle) \hfill (11.0) \\
20 & transform\_mix & Gaussian & $C_{\mathrm{exp}}$, stationary & $10\,+ \log(1{+}s_1) + e^{-s_1/2} + s_1/(1{+}s_2)$ \hfill (11.0) \\
21 & nonsmooth      & Gaussian & $C_{\mathrm{exp}}$, stationary & $10\,+ \mathrm{hinge}(s_1{-}5) + |s_2 - 4|$ ($C^0$ creases) \hfill (11.0) \\
\bottomrule
\end{tabular}
\end{table}
...
(method, dataset), so all $K$-comparisons are \emph{paired}. The
$\Kmrts \in \{30, 40, 50\}$ extension is now complete for every fitted
skew-$t$ setting (1, 3--12), so the full 12-point grid
$\{0, 3, \dots, 50\}$ underlies all their tables.
...
\input{brier_byK_table}
\input{brier_byK_table_c}
\input{brier_byK_table_b}
...
\section{The non-$t$ spotlight quartet: settings 16, 19, 20, 21}
\label{sec:spotlight}

\subsection*{Shared construction}

\begin{itemize}
\item All four settings draw
\begin{equation}
Y_t(\bs) \;=\; 10 \;+\; g(\bs) \;+\; \sigma_g\,\varepsilon_t(\bs),
\qquad \varepsilon_t \stackrel{iid}{\sim} N(\boldsymbol 0,\, C_{\mathrm{exp}}),
\quad \sigma_g = 4,\quad t = 1,\dots,50,
\label{eq:nontdgp}
\end{equation}
so the marginal law is Gaussian and every $t$-based error model is
misspecified. Methods, holdout and scoring are as in Section~1; the fitted
grid is $\Kmrts \in \{0, 30, 40, 50\}$ with $n = 3$, except settings 16 and 20,
which are at $n = 30$ on the full $K$ grid $\{0,3,5,8,10,12,15,20,25,30,40,50\}$.
\item Each mean surface is the image of a raw field
$\tilde g \in \R^{144}$ under the orthogonalise-and-standardise map
\begin{equation}
g \;=\; 11\,\frac{(I - P_X)\,\tilde g}
{\widehat{\mathrm{sd}}\{(I - P_X)\tilde g\}},
\qquad P_X = X(X^\top X)^{-1}X^\top,\quad
X = [\boldsymbol 1,\, s_1,\, s_2] \in \R^{144 \times 3},
\label{eq:orthostd}
\end{equation}
so that $g \perp \operatorname{col}(X)$ and
$\widehat{\mathrm{sd}}(g) = 11$ exactly. Hence the $K = 0$ design spans
\emph{none} of $g$; composite raw terms are standardised to unit sd before
summing, written $\mathrm{std}\{\cdot\}$ below.
\item Recoverability is quantified before any MCMC by the noise-free
out-of-sample projection error
\begin{equation}
e(K) \;=\;
\frac{\lVert B_K^{\mathrm{te}}\hat\beta_K - g^{\mathrm{te}}\rVert_2}
     {\lVert g^{\mathrm{te}} - \bar g^{\mathrm{te}}\rVert_2},
\qquad
\hat\beta_K \;=\; \operatorname*{arg\,min}_{\beta \in \R^{K}}
\lVert B_K^{\mathrm{tr}}\beta - g^{\mathrm{tr}}\rVert_2,
\label{eq:projerr}
\end{equation}
where $B_K$ is the $K$-column MRTS basis built on the 100 training sites
(superscripts: evaluated at the training / 44 test sites).
Table~\ref{tab:spotlight} reports $e(K)$ together with the oracle Brier
floor $R^\star$ recomputed under \eqref{eq:nontdgp}.
\end{itemize}

\begin{table}[ht]
\centering\small
\caption{The spotlight quartet: noise-free projection error $e(K)$ of
\eqref{eq:projerr} and oracle Brier floor $R^\star$ (Monte Carlo, Gaussian
error, $\sigma_g = 4$; \texttt{nont\_precheck.R}).}
\label{tab:spotlight}
\begin{tabular}{llccccl c}
\toprule
ID & \texttt{surf\_type} & $e(5)$ & $e(15)$ & $e(30)$ & $e(50)$ &
$K \to \infty$ behaviour & $R^\star$ \\
\midrule
16 & sigmoid\_front & 0.591 & 0.250 & 0.131 & 0.094 & $\downarrow$, elbow at $K \approx 40$--$50$ & 0.559 \\
19 & poly2          & 0.226 & 0.054 & 0.030 & 0.015 & $\downarrow$, saturated by $K \approx 12$   & 0.286 \\
20 & transform\_mix & 0.895 & 0.506 & 0.272 & 0.214 & $\downarrow$, slowest decay                & 0.396 \\
21 & nonsmooth      & 0.469 & 0.159 & 0.074 & 0.063 & plateau $\approx 0.063 > 0$                & 0.431 \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{Setting 16 (\texttt{sigmoid\_front}): the steepest recoverable
front}

\begin{itemize}
\item With $r(\bs) = \lVert \bs - (2,2) \rVert_2$ and width $w = 1$,
\begin{equation}
\tilde g_{16}(\bs) \;=\; \tanh\!\big(w^{-1}\{r(\bs) - 4.5\}\big).
\label{eq:g16}
\end{equation}
\item Since $\lVert \nabla \tilde g_{16} \rVert
= w^{-1}\operatorname{sech}^2\!\big(w^{-1}\{r - 4.5\}\big) \le w^{-1}$, the
transition is confined to the annulus $|r - 4.5| \lesssim w$, of width
$\approx 2w = 2$ units. The width is calibrated to the site-density
resolution cap of Section~5: $e(K)$ decays monotonically with no
extrapolation blow-up (Table~\ref{tab:spotlight}).
\item Role: the sharpest \emph{smooth} localised structure the 100-site
training density supports --- the fine-$K$ stress test for mean recovery
under a misspecified error law.
\end{itemize}

\subsection*{Setting 19 (\texttt{poly2}): the easy-end control}

\begin{itemize}
\item With rescaled coordinates $(x_1, x_2) = \bs / 10$,
\begin{equation}
\tilde g_{19}(\bs) \;=\;
\mathrm{std}\big\{(x_1 - \tfrac12)^2 + (x_2 - \tfrac12)^2\big\}
\;+\; \mathrm{std}\big\{(x_1 - \tfrac12)(x_2 - \tfrac12)\big\}.
\label{eq:g19}
\end{equation}
\item $\tilde g_{19}$ lies in the 6-dimensional space $\mathcal P_2$ of
quadratic polynomials, and $(I - P_X)$ quotients out its affine part, so the
target is the 3-dimensional pure-quadratic component
$\operatorname{span}\{s_1^2, s_1 s_2, s_2^2\} \bmod \operatorname{col}(X)$.
Accordingly $e(K)$ collapses fastest of all nine non-$t$ surfaces
($0.23 \to 0.05 \to 0.015$ at $K = 5, 15, 50$).
\item Role: control setting --- a method that fails here fails for
likelihood or sampler reasons, not basis-resolution reasons.
\end{itemize}

\subsection*{Setting 20 (\texttt{transform\_mix}): boundary-concentrated
curvature}

\begin{itemize}
\item
\begin{equation}
\tilde g_{20}(\bs) \;=\; \mathrm{std}\{\log(1 + s_1)\}
\;+\; \mathrm{std}\{e^{-s_1/2}\}
\;+\; \mathrm{std}\{s_1 / (1 + s_2)\}.
\label{eq:g20}
\end{equation}
\item Every term is monotone in $s_1$ with curvature maximal on the domain
boundary, e.g.\ $\partial^2_{s_1} \log(1 + s_1) = -(1 + s_1)^{-2}$, largest
at $s_1 = 0$ --- exactly where the uniform site draw is sparsest. Hence the
slowest $e(K)$ decay of the quartet ($0.21$ at $K = 50$).
\item Role: the transformations an applied modeller actually writes down
(log, exponential decay, ratio); MRTS must spend its resolution where the
data density is lowest.
\end{itemize}

\subsection*{Setting 21 (\texttt{nonsmooth}): structural
non-representability}

\begin{itemize}
\item With $(x)_+ = \max(x, 0)$,
\begin{equation}
\tilde g_{21}(\bs) \;=\; \mathrm{std}\{(s_1 - 5)_+\}
\;+\; \mathrm{std}\{|s_2 - 4|\}.
\label{eq:g21}
\end{equation}
\item $\tilde g_{21} \in C^0 \setminus C^1$: its gradient jumps across the
crease lines $\{s_1 = 5\}$ and $\{s_2 = 4\}$, while every MRTS column is a
$C^\infty$ thin-plate-spline eigenfunction. The projection error therefore
\emph{plateaus} instead of vanishing:
$e(30) = 0.074$, $e(40) = 0.064$, $e(50) = 0.063$.
\item Role: the mean-channel counterpart of the dependence settings 3--8 ---
structure that \emph{no} $K$ can fully represent, bounding the best MRTS fit
away from the oracle at every $K$.
\end{itemize}

\subsection*{What the spotlight settings show}

Settings 16 and 20 are now fitted at $n = 30$ on the full $K$ grid; 19 and 21
remain $n = 3$ pilots. The $n = 30$ pair carries the weight of what follows.

\begin{itemize}
\item \textbf{Unthresholded methods gain across the board.} On both $n = 30$
settings the Gaussian and skew-$t$ methods reduce all three scores: setting 16
to $0.94$--$0.96$ (energy/variogram) and $0.93$--$0.95$ (Brier); setting 20
more strongly, with skew-$t$ K5 the standout ($0.70$ energy, $0.63$ variogram,
$0.59$ Brier) (Tables~\ref{tab:energy}--\ref{tab:vario}, \ref{tab:brierKc}).
Under a misspecified error law the mean channel finally moves every score in
the same direction --- unlike the skew-$t$-core settings, where these methods
were flat.
\item \textbf{The thresholded methods split by surface --- and the $n = 3$
``inversion'' was largely small-sample noise.} At $n = 30$ the censored
methods are \emph{flat} on setting 16 (energy $1.00$/$1.00$, variogram
$1.00$/$1.00$ for the K1/K5 variants) --- \emph{not} the $1.5$--$1.7$ energy
\emph{loss} the $n = 3$ pilot had reported --- yet gain \emph{strongly} on
setting 20 (energy $0.49$/$0.52$, variogram $0.39$/$0.40$). The discriminator
is where the surface puts its curvature: setting 20 concentrates it where the
uniform sites are sparsest, and there the mean basis rescues the censored
likelihood; the smoother sigmoid front of setting 16 leaves nothing to rescue.
The larger energy-ratio excursions still visible at $n = 3$ (settings 14, 15,
17, up to $2.9$) should be read with that same small-$n$ caution.
\item \textbf{Brier improves for every method on both $n = 30$ settings}
($0.59$--$0.95$); the realised gains do not follow the $e(K)$ ordering and
stay far above the floor $R^\star$, both consistent with Finding~3's
sharpness--extrapolation account.
\end{itemize}

\section{In progress}

\begin{enumerate}
\item \textbf{Non-$t$ extension analysis (settings 13--21)}: the extension
exists because all earlier settings share the skew-$t$ core, so at large $K$
the fitted skew-$t$ is \emph{correctly specified} --- a confound for every
``MRTS helps/hurts'' conclusion. Its nine settings are now fitted and scored
($n = 30$ on the full $K$ grid for settings 16 and 20; $n = 3$ pilots for the
other seven; Tables~\ref{tab:energy}--\ref{tab:vario} and \ref{tab:brierKc};
four settings introduced in Section~\ref{sec:spotlight}), but the systematic
write-up --- in particular why the thresholded methods, the carriers of every
skew-$t$-core gain, \emph{lose} under some misspecified-error surfaces (energy
ratios up to $2.9$ at $n = 3$, settings 14, 15, 17) yet are flat on setting 16
and gain sharply on setting 20 at $n = 30$, while the unthresholded methods
post steady gains throughout --- is still to be done. Bringing the remaining
seven settings to $n = 10$ is the next step.
\item \textbf{$n$ upgrade}: settings 4--12 and seven of the non-$t$ block are
$n = 3$ pilots (settings 16 and 20 are already at $n = 30$); the headline
contrasts (11, 12, and the disambiguation) merit $n = 10$. An automated fill
script (fit increment $\to$ re-score $\to$ verify overlap against the pilot
cells $\to$ delete fits) performs each upgrade; it resumes cleanly after an
interruption by re-inventorying the missing $(m, d, K)$ cells.
\end{enumerate}
