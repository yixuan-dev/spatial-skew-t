...
so the skew-$t$ error law is misspecified) are fitted at pilot scale
($n=3$) and included in the score tables; their analysis is deferred.
\end{abstract}
...
error law at large $K$. All nine are fitted and scored at pilot scale
($n = 3$, $\Kmrts \in \{0, 30, 40, 50\}$) and appear in the score tables;
...
\bottomrule
\multicolumn{5}{p{0.92\textwidth}}{\footnotesize $^{a}$ setting 2 was fitted at $K=0$ only (no MRTS sweep); it is excluded from the score tables. $^{b}$ settings 13--21 use the grid $\Kmrts \in \{0,30,40,50\}$; all nine are fitted and scored ($n=3$).}
\end{tabular}
\end{table}
...
grid is $\Kmrts \in \{0, 30, 40, 50\}$ with $n = 3$ (the $n = 10$ / full-$K$
upgrade of settings 16 and 19 is running).
...
\subsection*{What the $n = 3$ pilots already show}

\begin{itemize}
\item The unthresholded methods post consistent gains on all four settings:
best energy ratios $0.93$--$0.97$, variogram $0.90$--$0.97$, Brier
$0.88$--$0.94$
(Tables~\ref{tab:energy}--\ref{tab:vario}, \ref{tab:brierKc}), with skew-$t$
K5 on setting 20 the standout ($0.71$ energy, $0.60$ Brier). Under a
misspecified error law the mean channel finally moves all three scores in
the same direction --- unlike the skew-$t$-core settings, where the
unthresholded methods were flat.
\item The thresholded methods \emph{invert} their skew-$t$-core role: on
settings 16 and 21 their best-over-$K$ energy ratios exceed 1 (respectively
$1.53$/$1.72$ and $1.05$/$1.21$ for the K1/K5 variants), and on 19 the K1
variant loses too ($1.34$). Setting 20 is the lone exception ($0.60$ K1,
$0.45$ K5) --- the only spotlight surface where the censored likelihood
still profits from a mean basis.
\item Yet the same thresholded fits \emph{improve} their Brier scores
everywhere ($0.85$--$0.91$), so their failure is a predictive-field
(energy/variogram) phenomenon, not a tail-probability one. The realised
gains also do not follow the $e(K)$ ordering, and every Brier ratio stays
far above its floor $R^\star$ --- both consistent with Finding~3's
...
at pilot scale ($n=3$; Tables~\ref{tab:energy}--\ref{tab:vario} and
\ref{tab:brierKc}; four settings introduced in
Section~\ref{sec:spotlight}), but the systematic write-up --- in
particular why the
thresholded methods, the carriers of every skew-$t$-core gain, \emph{lose}
under several misspecified-error surfaces (energy ratios up to $2.9$,
settings 14--17) while the unthresholded methods post steady 4--7\% gains ---
is still to be done.
\item \textbf{$n$ upgrade}: settings 4--12 and the non-$t$ block are $n=3$
pilots; the headline contrasts (11, 12, and the disambiguation) merit
$n = 10$. An automated fill script
(\texttt{fill\_16\_19.ps1}: fit increment $\to$ re-score $\to$ verify
overlap $\to$ delete fits) is running settings 16 and 19 first.
\end{enumerate}
