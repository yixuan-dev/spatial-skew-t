...
Settings 16 and 20 are now fitted at $n = 10$ on the full $K$ grid; 19 and 21
remain $n = 3$ pilots. The $n = 10$ pair carries the weight of what follows.

\begin{itemize}
\item \textbf{Unthresholded methods gain across the board.} On both $n = 10$
...
$0.60$ Brier) (Tables~\ref{tab:energy}--\ref{tab:vario}, \ref{tab:brierKc}).
...
``inversion'' was largely small-sample noise.} At $n = 10$ the censored
...
setting 20 (energy $0.53$/$0.53$, variogram $0.42$/$0.43$). The discriminator
...
\item \textbf{Brier improves for every method on both $n = 10$ settings}
($0.60$--$0.95$); the realised gains do not follow the $e(K)$ ordering and
...
($n = 10$ on the full $K$ grid for settings 16 and 20; $n = 3$ pilots for the
...
and gain sharply on setting 20 at $n = 10$, while the unthresholded methods
...
$n = 3$ pilots (settings 16 and 20 are already at $n = 10$); the headline
