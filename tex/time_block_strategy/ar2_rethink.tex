...
\paragraph{Symptom.}
Every cell of the AR(2) method's predictive sample $\hat y$ was
\texttt{NaN} -- $M \cdot n_s \cdot H \cdot B$ entries, zero
finite values -- and the NaN avalanche tripped
\texttt{checkNumeric} inside
\texttt{scoringRules::crps\_sample}. The i.i.d.\ method (which
uses a closed-form Gibbs update for $z$, bypassing the copula) was
unaffected, which is what isolated the failure to the AR(2) branch.

\paragraph{Root cause.}
The half-normal copula transform applied to the latent skewness
process is
\begin{equation*}
  ...
  \Phi^{-1}(0) = -\infty .
\end{equation*}
The backend default \texttt{z.init = 0} therefore feeds
$z^\star = -\infty$ to the very first iteration of
\texttt{updateZTS\_AR2}. The Metropolis log-ratio then evaluates the
Gaussian transition kernel
\texttt{ar2\_conditional\_log\_density} at $-\infty$, where its
exponent reduces to $\infty - \infty = \mathrm{NaN}$. The acceptance
step is guarded by \texttt{if (!is.na(R))} in
\texttt{update\_params\_ar2.R}, which silently \emph{skips} every
proposal when $R$ is \texttt{NaN} -- so the chain never leaves
$z \equiv 0$ and $\phi_z \equiv \bm 0$ for any iteration, while
$\lambda$ becomes unidentified ($\lambda \cdot z = 0$ regardless of
$\lambda$) and its posterior wanders over $[-47, 38]$ despite the
true $\lambda = 3$.

In \texttt{forecast\_block} the seam state is then
$z^\star_{T_o} = -\infty$, and the AR(2) recursion produces
\begin{equation*}
  \phi_{z,1}\cdot(-\infty) + \phi_{z,2}\cdot(-\infty)
  = 0\cdot(-\infty) = \mathrm{NaN}
\end{equation*}
because $\phi_z = \bm 0$ exactly; the \texttt{NaN} cascades through
the inverse copula and the spatial draw into every cell of
$\hat y$.

\paragraph{Fix.}
Pass \texttt{z.init = 0.01} explicitly to \texttt{mcmc()} in
\texttt{run-settings.R}. The first iteration's \texttt{hn.cop} then
evaluates finitely, the MH ratio is real-valued, the chain mixes,
and the imputed $z$, $\phi_z$, and $\lambda$ trajectories become
non-degenerate.

\paragraph{Backend follow-up.}
The silent acceptance guard is the deeper defect: a \texttt{NaN}
log-ratio is not a probabilistic rejection but a numerical failure,
and conflating the two suppresses the diagnostic signal that
matters most. A defensive patch in \texttt{updateZTS\_AR2} should
either count \texttt{NaN} as a rejection \emph{with} a
\texttt{warning("MH ratio NaN; chain may be stuck")}, or refuse
\texttt{z.init <= 0} when \texttt{temporalz = TRUE} -- so the user
sees the failure rather than absorbing it.

\paragraph{Verification.}
The $\phi = \bm 0$ collapse diagnostic of
Remark~\ref{rem:checks}~(2) is the canonical check, but only if the
imputed $z$ trajectory is inspected \emph{before} forecasting:
\texttt{range(fit\$z)} stuck at $[0,0]$ is the signature, and
\texttt{Var(fit\$lambda)} of order $10^2$ alongside truth $\lambda
= 3$ is the corroborating tell. Both can be checked in one line per
fit and added to the post-fit health screen of
Remark~\ref{rem:checks}.
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Silent acceptance is the worst kind of failure.} A
        Metropolis update that returns \texttt{NaN} must either fail
        loudly or count as a rejection \emph{with} a warning, but
        never be hidden by an \texttt{is.na} gate; Incident A2 was
        undetectable at the fit stage and produced a structurally
        well-formed output array.
