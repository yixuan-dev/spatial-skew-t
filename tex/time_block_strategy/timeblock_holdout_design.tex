...
\label{sec:forecast-pipeline}

\paragraph{Forecast pipeline at a glance.}
For each held-out block $b$ with seam $T_o = T_o^{(b)}$ and horizon
$H$, the predictive draw $\hat y^{(m)}(s,\,T_o+h)$ at lead $h$ is
built in \emph{five} stages, once per MCMC sample $m=1,\dots,M$.

\begin{enumerate}[label=\textbf{(\arabic*)},leftmargin=2.4em,
                  itemsep=3pt,topsep=2pt]
  \item \emph{Fit on the contiguous prefix.} The training data
        $y(\cdot,\,t\le T_o)$ feeds the MCMC, which returns
        $M$ joint posterior draws $\theta^{(m)}$ together with imputed
        observable latent trajectories
        $\tau^{(m)}_{k,\cdot},\,z^{(m)}_{k,\cdot},\,w^{(m)}_{k,\cdot}$
        for every $t \le T_o$. All sites are observed; the holdout
        is purely temporal.
  \item \emph{Seam state on the Gaussian scale.} The two columns of
        draw $m$'s trajectory at $t \in \{T_o-1,\,T_o\}$ are mapped
        \emph{back} to the latent scale by the forward copula
        transforms,
        \begin{equation*}
          \tau^{\star(m)}_{k,t}
            = \Phi^{-1}\!\big(G_{\alpha^{(m)}/2,\,\beta^{(m)}/2}(\tau^{(m)}_{k,t})\big),
          \qquad
          z^{\star(m)}_{k,t}
            = \Phi^{-1}\!\big(H_{\sigma_z}(z^{(m)}_{k,t})\big),
          \quad \sigma_z = (\tau^{(m)}_{k,t})^{-1/2}.
        \end{equation*}
        The state is taken from draw $m$'s own imputed trajectory,
        \emph{never} from the stationary law $\Norm(\bm 0, \Sigma)$
        -- Corollary~\ref{cor:stationary-limit} is precisely the
        warning against the latter.
  \item \emph{AR(2) recursion for $h = 1,\dots,H$.} On the Gaussian
        scale, with innovation variance fixed by Yule--Walker
        \eqref{eq:yw} so the marginal variance stays $1$,
        \begin{equation*}
          X^{\star(m)}_{k,T_o+h}
        ...
        \end{equation*}
        applied independently to $\tau^\star$ and $z^\star$ with
        their own $\phi^{(m)}$. The i.i.d.\ baseline replaces this
        step by $X^{\star(m)}_{k,T_o+h} \overset{\text{iid}}{\sim}
        \Norm(0,1)$ at every lead -- the infinite-horizon limit
        (Corollary~\ref{cor:stationary-limit}).
  \item \emph{Inverse copula back to observables.} Apply
        \eqref{eq:tau-transform}--\eqref{eq:w-transform} to the
        forecasted latent slice; the $\tau$--$z$ coupling
        \eqref{eq:z-transform} is enforced \emph{within} draw $m$,
        i.e.\ the half-normal scale $\sigma_z$ uses the same draw's
        forecasted $\tau$, not an independent marginal.
  \item \emph{Spatial draw.} For each lead $h$ (time
        $t = T_o + h$),
        \begin{equation*}
          \hat y^{(m)}(s,t)
          = x(s,t)^\top\beta^{(m)}
          + \lambda^{(m)} z^{(m)}_{g(s,t),\,t}
          + \varepsilon^{(m)}(s,t),
          \qquad
          \varepsilon^{(m)}(\cdot,t) \sim
            \Norm\!\Big(\bm 0,\,(\tau^{(m)}_{g(s,t),\,t})^{-1} C^{(m)}\Big),
        \end{equation*}
        with $C^{(m)}$ the spatial correlation matrix at
        $(\rho^{(m)},\nu^{(m)},\gamma^{(m)})$.
\end{enumerate}
Averaging $\hat y^{(m)}$ over $m=1,\dots,M$ integrates parameter
\emph{and} seam-state uncertainty jointly
(Proposition~\ref{prop:coherence}); the per-block output is the
predictive array of shape $M \times n_s \times H$. The full
pseudocode is Algorithm~\ref{alg:forecast}.

\paragraph{Non-negotiable design choices.}
The five stages above encode three constraints derived earlier and
not negotiable in any reimplementation:
...
% ============================================================
\section{Implementation Audit Log}
\label{sec:audit}

This section records the implementation defects encountered while
operationalizing Section~\ref{sec:redesign} into runnable code, the
root cause of each, the patch applied, and the diagnostic of
Remark~\ref{rem:checks} that did or should have caught it.

\subsection{Incident A1: dimension drop in \texttt{fit\$tau} when $K=1$}
\label{sec:audit-dim}

\paragraph{Symptom.}
\texttt{forecast\_block} halted with ``\texttt{incorrect number of
dimensions}'' at the seam-state slice
\texttt{tau\_traj[m, , seam\_cols]}.

\paragraph{Root cause.}
The MCMC backend returns the latent trajectory via
\texttt{tau = keepers.tau[return.iters, , ]} without
\texttt{drop = FALSE}. With \texttt{nknots = 1} the singleton knot
axis is silently dropped, so \texttt{fit\$tau} arrives as a 2-D
matrix $[M, T_o]$ rather than the documented 3-D array
$[M, K, T_o]$, and every index expression written for the 3-D shape
breaks.

\paragraph{Fix.}
Restore the knot axis at the top of \texttt{forecast\_block}: if
\texttt{length(dim(tau\_traj)) == 2}, wrap to
\texttt{array(tau\_traj, dim = c(M, 1, T\_o))}. Extract the seam
slice with \texttt{drop = FALSE} and reshape via
\texttt{matrix(\dots, K, 2L)} so the $K = 1$ case yields a
$K \times 2$ matrix (not a vector) when passed to
\texttt{ar2\_recurse}, whose body indexes \texttt{seam\_state[, j]}.

\paragraph{Verification.}
The $H = 0$ parity diagnostic of Remark~\ref{rem:checks}~(1) is
designed to expose exactly this class of seam-indexing bug: a
non-zero AR(2) versus i.i.d.\ score split at lead $0$ is the
signature.

\subsection{Incident A2: $z \equiv 0$ under \texttt{temporal}/\texttt{ar2} updates}
\label{sec:audit-zinit}

\paragraph{Correction (July 2026).}
The resolution recorded in the original draft of this section --- ``pass
\texttt{z.init = 0.01}'' --- was wrong. That argument was not the fix; it
\emph{was} the bug. It converted a loud, self-announcing failure into a
silent one that corrupted a full study before being caught. The corrected
account is below; the complete forensic write-up is the standalone note
\texttt{tex/z\_init\_bug}.

\paragraph{Symptom (two faces).}
Under the true backend default \texttt{z.init = 0} every cell of the AR(2)
predictive $\hat y$ was \texttt{NaN} --- a \emph{loud} failure that halts
\texttt{scoringRules::crps\_sample} on contact. The i.i.d.\ method (a
closed-form Gibbs update for $z$ that bypasses the copula) was unaffected,
which isolated the failure to the AR(2) branch. Passing
\texttt{z.init = 0.01} removed the \texttt{NaN}; it did \emph{not} remove
the failure. It replaced the loud face with a \emph{silent} one: a chain
that mixed, a posterior that concentrated, a fitted mean that matched the
data, and a predictive standard deviation of $104$ against a data marginal
of $4.38$ --- publishable-looking scores built on a corrupted fit.

\paragraph{Root cause of the loud face.}
The half-normal copula transform on the latent skewness process is
\begin{equation*}
  z^\star_{k,t} = \Phi^{-1}\!\big(H_{\sigma_z}(z_{k,t})\big),
  \qquad
  H_{\sigma_z}(0) = 0
  \;\Longrightarrow\;
  \Phi^{-1}(0) = -\infty .
\end{equation*}
A start at $z = 0$ feeds $z^\star = -\infty$ to the first iteration of
\texttt{updateZTS\_AR2}; the Metropolis log-ratio evaluates to
$\infty - \infty = \mathrm{NaN}$; and the acceptance guard
\texttt{if (!is.na(R))} in \texttt{update\_params\_ar2.R} silently
\emph{skips} every such proposal, so $z \equiv 0$ and $\phi_z \equiv \bm 0$
freeze for all iterations and the \texttt{NaN} cascades into $\hat y$.

\paragraph{Why \texttt{z.init = 0.01} was the real bug.}
It moves the start off the singularity but not into the bulk:
$\operatorname{hn.cop}(0.01, 1) = \Phi^{-1}(2\Phi(0.01)-1) = -2.41$, a
$2.4\sigma$ tail on the very scale the chain must explore. A random-walk
Metropolis chain launched there does not return: across $20{,}000$
iterations $z$ stays near $0.2$ while its own model law $\hn(0,\sigma)$,
$\sigma\approx 2$, demands $\E[z]=\sigma\sqrt{2/\pi}\approx 1.6$. A frozen,
near-constant $z$ looks maximally persistent, so $\hat\phi_{1z}$ is driven
toward a unit root ($1.11$ vs.\ truth $0.80$), which pins $z$ harder still.
Because the data see only the product $\lambda z$, $\lambda$ then
compensates along the ridge, running to $-97.8$ (truth $+3$) with
$\beta_0 = 36.4$ (truth $10$), all while reproducing the data mean so the
likelihood looks fine from inside the fit. \texttt{forecast\_block}
correctly redraws $z$ at its proper scale ($\approx 1.6$), multiplies by
the corrupted $\lambda$, and the predictive detonates: SD $104$, median far
negative, for data that never leave $14.5\pm4.4$. The entire ``AR(2) is
$4\times$ worse than i.i.d.''\ headline was this one argument.

\paragraph{The real fix.}
\emph{Remove} \texttt{z.init} from the \texttt{mcmc()} call in
\texttt{run-settings.R} and let the backend default apply. That default was
itself corrected: \texttt{mcmc\_ar2.R:249--250} now sets, when
\texttt{z.init} is \texttt{NULL},
\begin{equation*}
  \texttt{z.init} = 0.6745\,/\!\sqrt{\texttt{tau.init}},
\end{equation*}
the \emph{median} of $\hn(0,\texttt{tau.init}^{-1/2})$. Since
$0.6745 = \Phi^{-1}(0.75)$, this maps to
$z^\star = \Phi^{-1}(2\Phi(0.6745)-1) = \Phi^{-1}(0.5) = 0$ --- dead centre
of the copula scale, finite for any $\tau$. On the exact chains that
previously exploded, deleting the argument restores $\lambda = +2.80$,
$\beta_0 = 10.36$, $\bar z = 1.68$, $\phi_{1z} = 0.62$, and the lead-15
predictive SD to $4.94$ against a marginal of $4.38$ --- the forecast
converges to climatology, as an AR forecast must. Every score computed
under the old workaround must be discarded and recomputed.

\paragraph{Verification.}
The check that would have caught it needs no knowledge of the
initialisation: the model asserts $z\sim\hn(0,\tau^{-1/2})$, hence
$\E[z]=\tau^{-1/2}\sqrt{2/\pi}$, and the fitted $\bar z$ was $7\times$
smaller --- a fit that violates its own generative law by a factor of
seven is not a fit. In simulation the second check is free: truth is
$\lambda=+3$ and the fit returned $-98$, so the sign alone is
disqualifying. Both are one line per fit; either fails on the first block
in minutes rather than after a full study. These are now the standing
assertions of \texttt{fit\_diag\_utils.R} (Remark~\ref{rem:checks}), and
the residual $(\beta_0,\lambda,z)$ ridge they guard against is characterised
in \texttt{tex/lambda\_phiz\_ridge}.

\subsection{Incident A3: \texttt{parLapply} missing \texttt{tasks} export}
\label{sec:audit-psock}

\paragraph{Symptom.}
The driver halted with ``\texttt{object 'tasks' not found}'' inside
the PSOCK workers whenever \texttt{workers > 1}; the single-worker
branch ran cleanly with the same input.

\paragraph{Root cause.}
The \texttt{parLapply} body references \texttt{tasks\$method[i]} and
\texttt{tasks\$dataset[i]}, but \texttt{tasks} was missing from the
\texttt{clusterExport} call. PSOCK workers serialize the function
with its closure, but free-name lookups on the worker happen against
the worker's global environment, not the closure -- so the symbol
is unbound there. The single-worker branch evaluates the body in
the script's own environment, which masks the bug entirely.

\paragraph{Fix.}
Add \texttt{"tasks"} to the \texttt{clusterExport} list. Every free
name the worker body references must be either passed as a
function argument or exported explicitly.

\paragraph{Verification.}
A two-worker, two-task smoke run before scaling out exposes this
class of bug in seconds; serial-only verification cannot.

\subsection{Lessons}

\begin{itemize}[leftmargin=1.4em]
  \item \textbf{A workaround that removes the symptom, not the cause,
        makes the next failure quieter.} Incident A2's original
        ``fix'' silenced a loud \texttt{NaN} avalanche and in doing so
        buried a chain-freeze that produced a structurally well-formed
        but corrupted output array --- undetectable at the fit stage and
        fatal only at forecast time. The lesson is twofold: a Metropolis
        update returning \texttt{NaN} must warn rather than be hidden by
        an \texttt{is.na} gate, and a patch that makes an error message
        disappear is not a fix until the underlying quantity is checked.
  \item \textbf{Singleton dimensions are platform-specific landmines.}
        R drops them by default; passing \texttt{drop = FALSE} or
        restoring the axis explicitly is the only portable way to
        write $K$-agnostic forecast code (Incident A1).
  \item \textbf{Test the parallel path on a small grid.} PSOCK
        scoping bugs are invisible in serial mode (Incident A3); a
        two-worker, two-task run before fanning out is the cheapest
        possible insurance.
  \item \textbf{Augment Remark~\ref{rem:checks} with a pre-forecast
        fit audit.} Confirm
        $\Var(\mathrm{fit\$z}) > 0$,
        $\Var(\mathrm{fit\$phi.z}) > 0$, and
        $\mathrm{range}(\mathrm{fit\$lambda})$ concentrated near the
        prior mode \emph{before} invoking \texttt{forecast\_block}.
        A zero-variance $z$ trajectory is a sufficient (and very
        cheap) diagnostic for Incident A2.
\end{itemize}
