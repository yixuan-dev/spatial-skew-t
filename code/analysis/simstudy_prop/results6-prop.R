...
# results6-prop.R
#
# Analogue of ../simstudy/results6.R for the simstudy_prop fits.
#
# data setting (fixed): 6 - max-stable (mu=1, sig=1, xi=0.2, alpha=0.5, bw=1)
#
# Note: the prop method catalog (1..5) is misspecified for max-stable data,
# so this script scores prop fits as a robustness / misspecification check.
# The Morris baseline analogue is method 6 (Max-stable), which the prop
...
#
# prop basis ranks: 10, 20, 30, 40, 50 (matches settings_prop.csv grid).
# Missing fits are skipped with a "missing:" log line, so partial grids
# (e.g. p40 only) work fine.
#
...
#
# Outputs:
#   output/results/scores6-prop.RData
#########################################################################
...
datasets   <- 1:50
methods    <- 1:5
prop_ks    <- c(10, 20, 30, 40, 50)
fits_dir   <- "fits"
out_dir    <- "output/results"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
out_file   <- file.path(out_dir, "scores6-prop.RData")
