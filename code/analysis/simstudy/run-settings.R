...
# Usage: Rscript run-settings.R [--setting=<id>|--setting <id>] [datasets] [workers] [ms_threads] [methods]
# Note: --setting (if provided) must be the first argument after script name.
...
parse_setting_spec <- function(setting_str) {
    ...
    if (length(setting_ids) != 1) {
        stop("setting must evaluate to a single integer")
    }

    setting_id <- setting_ids[[1]]
    if (setting_id < 1) {
        stop("setting must be a positive integer")
    }
    ...
    if (!is.na(max_setting) && setting_id > max_setting) {
        stop(sprintf("setting must be in 1..%d", max_setting))
    }
...
extract_setting_arg <- function(args) {
    ...
    if (grepl("^--setting=", args[1])) {
        ...
        if (length(args) > 1) {
            ...
            remaining_args <- character(0)
        }
    } else if (args[1] == "--setting") {
        if (length(args) < 2) {
            stop("Missing value after --setting")
        }
        setting_value <- args[2]
        if (length(args) > 2) {
            remaining_args <- args[-c(1, 2)]
        } else {
            remaining_args <- character(0)
        }
    } else if (has_setting_anywhere) {
        stop("--setting must be the first argument after script name")
    }

    if (any(grepl("^--setting=", remaining_args)) || any(remaining_args == "--setting")) {
        stop("Specify --setting only once and place it first")
    }
