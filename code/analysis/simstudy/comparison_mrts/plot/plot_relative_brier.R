...
parse_pos_int <- function(raw, src) {
    ...
    if (!is.finite(val) || is.na(val) || val < 1L || val != as.numeric(raw)) {
        stop(sprintf(
            "%s must be a single positive integer (got '%s')",
            src, raw
        ), call. = FALSE)
    }
    val
...
cat(sprintf(
    "Setting = %d   MRTS k = %d   scores_dir = %s\n",
...
load_scores_cache <- function(setting_id, mrts_k, scores_dir) {
    ...
    if (length(miss) > 0L) {
        stop(sprintf(
            "Cache %s missing objects: %s",
            fpath, paste(miss, collapse = ", ")
        ), call. = FALSE)
    }
    list(
        file        = fpath,
...
verify_methods <- function(cache, needed) {
    ...
    if (length(miss) > 0L) {
        stop(sprintf(
            "Cache %s missing methods: %s",
            cache$file, paste(miss, collapse = ", ")
        ), call. = FALSE)
    }
}
...
plot_df <- rbind(
    make_panel_rows(
        relative_baseline, log_baseline,
        "method 2-5", NA_integer_
    ),
    make_panel_rows(
        relative_mrts, log_mrts,
...
data_out <- file.path(
    output_dir,
    sprintf(
        "setting%d_relative_brier_plot_data-K%d.csv",
...
plot_file <- file.path(
    output_dir,
    sprintf(
        "setting%d_relative_brier_method2to5_vs_mrts-K%d.png",
        setting_id, mrts_k_active
    )
)
...
mcol <- c(
    "gray30", "firebrick4", "dodgerblue4", "firebrick1",
    "dodgerblue1", "darkgreen", "purple4", "darkorange2"
)
mbg <- c(
    "gray70", "firebrick2", "dodgerblue2", "firebrick1",
    "dodgerblue1", "lightgreen", "plum", "moccasin"
)

png(filename = plot_file, width = 2600, height = 1250, res = 220)
# No per-panel main: shrink top margin; oma top reserves room for mtext.
par(mfrow = c(1, 2), mar = c(5.2, 6.2, 1.5, 1.5), oma = c(0, 0, 3, 0))

y_all <- plot_df$relative_brier[is.finite(plot_df$relative_brier)]
if (length(y_all) == 0) y_all <- c(1)
y_lim <- range(c(y_all, 1), na.rm = TRUE)
...
axis_labels <- ifelse(axis_at %in% axis_show,
    formatC(axis_at, format = "f", digits = 2), ""
)
...
if (nonpos_count > 0) {
    cat(sprintf(
        "Warning: %d non-positive relative-Brier values; log set to NA.\n",
        nonpos_count
    ))
}
