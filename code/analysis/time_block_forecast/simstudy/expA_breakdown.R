...
for (S in setting_ids) {
  f <- tbf_score_cache_file(S, data_suffix, prior_tag)
  if (!file.exists(f)) {
    stop(sprintf("score cache not found: %s (run scores.R --prior=%s)",
      f, prior_tag), call. = FALSE)
  }
...
write_body <- function(lines, file) {
  # The prior arm is stamped into every fragment: the table filenames are
  # not yet tagged (only one arm has been fitted), so this is what tells a
  # reader which campaign a table came from.
  writeLines(c(
    "% Auto-generated by expA_breakdown.R -- do not edit by hand.",
    sprintf("%% lambda-prior arm: %s (%s)", prior_tag,
      if (identical(prior_tag, "hn")) "HN(0, 20), --hn" else "N(0, 20), default"),
    lines), file)
  cat("wrote ", file, "\n", sep = "")
}
