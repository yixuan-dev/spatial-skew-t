#########################################################################
# refit_seq_fixed.R -- FIX VALIDATION. FAITHFULLY reproduce the original run's RNG stream.
#
...
for (b in seq_len(nblocks_run)) {
  ...
  fit <- mcmc(
    ...
    # z.init INTENTIONALLY NOT PASSED: the backend default (mcmc_ar2.R:249-250)
    # is 0.6745/sqrt(tau.init), the HALF-NORMAL MEDIAN, which maps to z.star = 0.
    # run-settings.R overrode it with 0.01, which starts z.star at -2.41 (a 2.4-sigma
    # tail) and the chain never escapes -> z freezes at ~0.2 instead of ~1.6 -> lambda
    # compensates to -98 -> the forecast explodes. THIS LINE IS THE BUG.
...
save(fits, file = "refit_seq_FIXED_fits.RData")
cat("\nfits saved -> refit_seq_FIXED_fits.RData\n")
